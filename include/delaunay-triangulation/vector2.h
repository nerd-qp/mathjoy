...
class Vector2
{
    ...
    //std::vector<unsigned int> triangulates;

};

// making it inline so it wouldn't violate the one definition rule
template <>
inline float Vector2<float>::dist(const Vector2<float> &v) const { return hypotf(x - v.x, y - v.y);}

template <>
inline double Vector2<double>::dist(const Vector2<double> &v) const { return hypot(x - v.x, y - v.y);}
